...
/app
  ...
  /assets
    icons.svg         — SVG sprite sheet, embedded in index.html at build time
    (fonts)
```
...
- **Branch-off:** cubic bezier from the parent's X to the lane's X, curving over ~30px of Y. The parent is the spine for top-level lines, or another line's X for nested branches.
- **Merge-back:** mirror bezier returning to the parent's X at the end Y
...
Icons are rendered as inline SVG symbols. A single `<svg>` sprite sheet is embedded in `index.html` inside a hidden `<defs>` block. Each icon is a `<symbol>` with a stable ID. Station rendering references icons with `<use href="#icon-id" />`, keeping the DOM lean and icons styleable via CSS (`fill: var(--color-label)`).

### Starter Icon Set

| ID | Use |
|---|---|
| `airplane-takeoff` | Start of a trip |
| `airplane-landing` | End of a trip |
| `book` | Book span |
| `film` | Movie or TV show |
...
| `run` | Running / fitness |
| `bike` | Cycling |
| `pin` | Generic location / relocation |
| `heart` | Relationship milestone |
| `star` | Generic milestone (spine events) |
| `birthday-cake` | Birthday |

Icons are light/dark mode aware via `fill: currentColor` or a CSS token reference — no separate icon variants needed per mode.

New icons are added to the sprite sheet centrally. Events reference them by ID string — if an unknown ID is encountered the station renders without an icon rather than erroring.

### Icon Placement by Zoom

| Zoom | Icon position | Dot | Label |
|---|---|---|---|
