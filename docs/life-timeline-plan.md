...
Place `<circle>` elements at the correct `(laneX, timeToY(date))` for each visible event. Station labels are `<text>` elements positioned to avoid overlap. On hover, show a detail card (HTML `<div>` overlay — not SVG — for easier styling and scrolling).
...
- Responsive layout foundation: breakpoints, mobile zoom control, bottom sheet card shell
